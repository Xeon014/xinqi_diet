...
- `pages/food-history`：饮食历史选择页
- `pages/exercise-search`：运动选择页（搜索、分类、最近搜索、运动编辑底部弹窗）
- `pages/exercise-history`：运动历史选择页
...
- 首页已有饮食记录左滑“再记一次”：进入 `food-search` 的复制预填模式（携带 `mode=copy&recordId`），用户确认后再点 `添加` 新增。
- 在 `food-search` 选中食物后，不再跳转独立页面，而是从底部弹出食物编辑弹窗：
  - 新建场景按钮：`添加`
  - 编辑场景按钮：`完成编辑`
- 食物选择页左侧筛选：`最近记录 / 最近搜索 / 自定义 / 套餐 / 内置分类`
- “最近记录”标题右侧新增 `历史记录` 入口，进入 `food-history` 按日期选择旧记录，再回到 `food-search` 预填新增。
- 最近记录左滑当前只保留删除，不再提供“再记一次”。
...
- 首页已有运动记录左滑“再记一次”：进入 `exercise-search` 的复制预填模式（携带 `mode=copy&recordId`），用户确认后再点 `添加` 新增。
- 在 `exercise-search` 选中运动后，从同页底部弹出运动编辑弹窗：
  - 新建场景按钮：`添加`
  - 编辑场景按钮：`保存`
- 运动选择页左侧筛选：`最近运动 / 最近搜索 / 自定义 / 内置分类`
- “最近运动”标题右侧新增 `历史记录` 入口，进入 `exercise-history` 按日期选择旧记录，再回到 `exercise-search` 预填新增。
- 最近运动左滑当前只保留删除，不再提供“再记一次”。
...
- 饮食餐次头部支持点击折叠。
- 折叠后不显示具体食物明细，只在餐次名称右侧显示该餐次总热量。
- 运动分组当前不参与折叠，仍保持原有列表展示。
...
- `MONTH` 展示日级点；`YEAR / ALL` 由后端按月聚合返回趋势点。
- `ALL` 进入后不再由前端递归拉取所有分页数据。
