...
public class ExerciseRecordQueryService {
    ...
    List<ExerciseRecord> findRecordsByUserAndDate(Long userId, LocalDate date) {
        return exerciseRecordRepository.findByUserAndDate(userId, date);
    }

    List<ExerciseRecord> findRecordsByUserAndDateRange(Long userId, LocalDate startDate, LocalDate endDate) {
